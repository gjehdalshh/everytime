...
function cmtWindow_open(loginI_user, cmtI_user, i_cmt) {
	mw_temp_cmt.style.display = 'block'
	cmt_ins_div.style.display = 'none'
	checkUser(loginI_user, cmtI_user, i_cmt)
}
...
function checkUser(loginI_user, cmtI_user, i_cmt) {
	...
	if(loginI_user == cmtI_user) {
		var remove = `<div class="remove" onclick="delCmtBoard(${i_cmt})">
				댓글 삭제
			</div>`
		}
		div.innerHTML = 
